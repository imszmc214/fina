...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['invoice_id'], $_POST['amount_pay'])) {
    ...
    // Validate inputs
    if (empty($invoice_id) || $invoice_id === 'undefined') {
    ...
    if ($ap) {
        ...
        $amount = $ap['amount'];
        $remaining = $amount - $ap['amount_paid'];

        // Validate payment amount
        if ($amount_pay > $remaining) {
            die("Error: Payment amount exceeds remaining balance");
        }
        ...
        // Use PA- prefix to ensure reference_id is never undefined
        $reference_id = "PA-" . $invoice_id;
        ...
        $empty_document = ''; // Empty blob for document field

        // Insert into pa table - INCLUDE document field with empty value
        $insert_sql = "INSERT INTO pa (
            reference_id, account_name, requested_department, mode_of_payment, 
            expense_categories, amount, description, document, payment_due, from_payable, 
        ...
        die("Error: Invoice not found or not approved");
    }
...
<body class="bg-white">
    ...
    <div class="overflow-y-auto h-full px-6">
        ...
        <div class="flex-1 bg-white p-6 h-full w-full">
            <div class="w-full">
                ...
                <div class="mb-6 flex items-center gap-6 flex-wrap border-b border-gray-200">
                    ...
                    <!-- Filters -->
                    <form method="GET" action="payables.php" class="flex items-center gap-4 flex-wrap mt-4">
                        <input
                        type="text"
                        id="searchInput"
                                ...
                                if ($result->num_rows > 0) {
                                    while ($row = $result->fetch_assoc()) {
                                        ...
                                        if ($remaining > 0.00001) {
                                            // Ensure invoice_id is not undefined before creating JSON
        ...
        <div id="paymentModal" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center hidden z-50">
            <div class="bg-white rounded-lg shadow-lg p-6 w-96">
                ...
                <form method="POST" action="payables.php">
    ...
    function openPaymentModal(data) {
        console.log("Payment modal data:", data); // For debugging
        
        // Ensure invoice_id is not undefined
        if (!data.invoice_id || data.invoice_id === 'undefined') {
