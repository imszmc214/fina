...
$sql = "SELECT * FROM dr WHERE status = 'disbursed' AND amount > 0 AND LOWER(mode_of_payment) != 'ecash' ORDER BY id DESC";
        ...
        if (paginated.length === 0) {
            ...
            paginated.forEach(row => {
                ...
                tbody.innerHTML += `<tr class="hover:bg-violet-100">
                    ...
                    <td class="py-2 px-4">
                        <button onclick='openPaymentModal("${rowData}")' class='rounded-full px-2 py-1 text-violet-700 border border-violet-600 bg-violet-10 text-xs font-semibold'>View Details</button>
                    </td>
                </tr>`;
            });
        }
