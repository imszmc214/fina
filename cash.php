...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (isset($_POST['approve_id'])) {
    ...
    // Fetch the record from cash table
    $fetch_sql = "SELECT * FROM cash WHERE id = ?";
    $stmt_fetch = $conn->prepare($fetch_sql);
    $stmt_fetch->bind_param("i", $approveId);
    $stmt_fetch->execute();
    $result_fetch = $stmt_fetch->get_result();
    $cash_record = $result_fetch->fetch_assoc();
    $stmt_fetch->close();

    if ($cash_record) {
      $expense_category = $cash_record['expense_categories'];
      $mode_of_payment = $cash_record['mode_of_payment'];
      $amount = $cash_record['amount'];
      $requested_department = $cash_record['requested_department'];
      $description = $cash_record['description'];
      $document = $cash_record['document'];
      $payment_due = $cash_record['payment_due'];
      $account_name = $cash_record['account_name'];
      $reference_id = $cash_record['reference_id'];
      ...
      try {
          ...
          error_log("Disbursement journal entry created for $reference_id via cash.php");
      } catch (Exception $e) {
      ...
      $stmt_insert->bind_param("sssssdsss", 
        $reference_id, 
        $account_name, 
